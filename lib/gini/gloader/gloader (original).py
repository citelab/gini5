...
def findSubNet4Switch(myGINI, name):

    # Search through the VMs for the switch
    # We are looking for the subnet
    for vm in myGINI.vm:
        for intf in vm.interfaces:
            if name == intf.target:
                return intf.network

    # Seach through the Virtual Routers for the switch
    for vr in myGINI.vr:
        for intf in vr.netIF:
            if name == intf.target:
                return intf.network
...
def createVS(myGINI, switchDir):
    ...
    for switch in switches:
        ...
        if retcode == 0:
            startOut = open("stopit.sh", "w")
            startOut.write("docker network remove %s" % switch.name)
            startOut.close()
...
def createASwitch(rtname, swname, subnet):

    x,rnum = rtname.split("_")
    x,snum = swname.split("_")
    swname = "Switch_" + rnum + snum

    command = "docker network create %s --subnet %s" % (swname, subnet)
    q = subprocess.call(command, stdout=subprocess.PIPE)
...
def setupTapInterface(rtname, swname, brname):
    ...
    subprocess.call("brctl addif %s %s" % (tapname, brname))
...
def createVR(myGINI, options):
    ...
    for router in myGINI.vr:
        ...
        for nwIf in router.netIF:
            swname = nwIf.target
            isswitch = checkSwitch(myGINI, swname)
            if not isswitch:
                # Router is directly connected to the Machine
                # We need a switch here to make the connection
                swname, brname = createASwitch(router.name, swname, nwIf.network)
            tapname = setupTapInterface(router.name, swname, brname)
            if (tapname == "None"):
                print "Router %s: unable to create tap interface"  % router.name
                return False
            else:
...
def createVM(myGINI, options):
    ...
    for uml in myGINI.vm:
        ...
        command = "docker run -id --name %s " % uml.name

        ### ---- process the UML interfaces ---- ###
        # add network interfaces
        for nwIf in uml.interfaces:
            isswitch = checkSwitch(myGINI, nwIf.target)
            if isswitch:
                command += "--ip %s --network %s " % (nwIf.ip, nwIf.target)
            else:
                isrouter = checkRouter(myGINI, nwIf.target)
                if isrouter:
                    print "---------------- Create a hidden switch -----"
        ...
        if retcode == 0:
            ...
            os.chdir(subUMLDir)
            startOut = open("startit.sh", "w")
            startOut.write("docker exec -it %s sh" % uml.name)
            startOut.close()
            os.chmod("startit.sh",0755)
            print "[OK]"
            os.chdir(oldDir)

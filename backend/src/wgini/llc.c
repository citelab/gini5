   ...
   void IniNodeLlc(NodeType type) {
	   ...
	   if (type == NORMAL) {
	       for (id=1; id<=TotNodeNo; id++) {
		          ...
		          for (j=0; j<LLC_TO_MACH_BUF_SIZE; j++) {
		   ...
		   //establish the connection from llc to mach.
...
//=======================================================
   void FinalizeNodeLlc(NodeType type) {
	   ...
	   if (type == NORMAL) {
	       for (id=1; id<=TotNodeNo; id++) {
			      //cancel mach->llc connection.
				  conn_close(WGNnode(id)->nodeLlc->inf);
                  //cancel llc->mach connection.
				  ...
				  for (i=0; i<LLC_TO_MACH_BUF_SIZE; i++) {
...
//=======================================================
//       FUNCTION:   LlcUpPortRecv	 (thread, get pkt from Mach)
//=======================================================
    void LlcUpPortRecv(void* psConn) {
	   ...
	   while(1){
		   ...
		   if (frame == NULL) {
			   verbose(4, "[LlcUpPortRecv]:: Node %d received a NULL packet from the Mach.", psConnection->node_id);
		   }
		   else {
		       verbose(4, "[LlcUpPortRecv]:: Node %d receives a packet from the Mach.", psConnection->node_id);
		   }
...
//=======================================================
    void  LlcUpPortSend(MobileNode *node) {
	    ...
	    while(1) {
			...
			node->nodeLlc->ltou_buf_Rcount = (node->nodeLlc->ltou_buf_Rcount+1)%LLC_TO_MACH_BUF_SIZE;
		    pthread_mutex_unlock(&(node->nodeLlc->ltou_array_mutex));

			verbose(4, "[LlcUpPortSend]:: NODE %d: LLC layer sends a packet to the Mach.", node->Id);
			...
			// Send the 802.3 frame to the Machs.
...
//=======================================================
    void LlcDownPortRecv(MobileNode *node, WGN_802_11_Mac_Frame *frame) {
	   ...
	   if (node->nodeLlc->LToUArray[node->nodeLlc->ltou_buf_Wcount]==NULL) {
		   ...
		   node->nodeLlc->ltou_buf_Wcount = (node->nodeLlc->ltou_buf_Wcount+1)%LLC_TO_MACH_BUF_SIZE;
	   }
	   else
	       fullArray=TRUE;
	   pthread_mutex_unlock(&(node->nodeLlc->ltou_array_mutex));
	   ...
	   if(fullArray==TRUE) {
          verbose(4, "[LlcDownPortRecv]:: NODE %d: LLC to Mach buffer is full, packet thrown.", node->Id);
	      Dot11FrameFree(frame);
...
//=======================================================
//       FUNCTION:   conn_open
//       PURPOSE:    Setup an individual connection between Mach and WGINI
//=======================================================
void conn_open(MobileNode *node,char* pchSocket){
	struct vpl_data *psMachConn;
	...
	// Set up Mach connection parameters
	psMachConn = (struct vpl_data *) malloc(sizeof(struct vpl_data));
	bzero(psMachConn,sizeof(struct vpl_data));
	psMachConn->sock_type = "unix";
	psMachConn->ctl_sock = pchSocket;
	psMachConn->ctl_addr = new_addr(psMachConn->ctl_sock, strlen(psMachConn->ctl_sock) + 1);
	psMachConn->data_addr = NULL;
	psMachConn->fd = -1;
	psMachConn->control = -1;
	name.zero = 0;
	name.pid = getpid();
	gettimeofday(&sWaitTime, NULL);
	name.usecs = sWaitTime.tv_usec;
	psMachConn->local_addr = new_addr(&name, sizeof(name));
	// Connect to the Mach
	if ((psMachConn->fd = vpl_connect(psMachConn)) < 0) {
		 free (psMachConn);
		 printf("[LlcProcess]:: The connection from Mach to LLC is failed.\n");
		 return;
	}
	...
	psRet->iDescriptor = psMachConn->fd;
	memcpy(&psRet->sVPLdata, psMachConn, sizeof(struct vpl_data));
	free(psMachConn);
...
void SocketNameGen(int id, char* sktname) {
    ...
    strcpy(sktname,getenv("GINI_HOME"));
		strcat(sktname,"/data/mach_virtual_switch/");

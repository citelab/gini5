...
# Set up consul-template, this is copy pasted from hashicorp's Dockerfile
...
ADD "includes/consul-template" "/bin/consul-template"
...
COPY includes/docker-entrypoint.sh /usr/local/bin/docker-entrypoint.sh

# consul-template config file and haproxy config template
COPY includes/consul-template-haproxy.conf /tmp/haproxy.conf
COPY includes/haproxy.ctmpl /tmp/haproxy.ctmpl

ENTRYPOINT ["docker-entrypoint.sh"]
# Run consul-template by default
CMD ["/bin/consul-template", "-config=/tmp/haproxy.conf", "-consul=consul:8500"]

# command for consul-template 0.20.0
# CMD ["/bin/consul-template", "-config=/tmp/haproxy.conf", "-consul-addr=consul:8500"]
